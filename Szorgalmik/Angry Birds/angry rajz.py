...
def main():
    ...
    V = turtle.numinput('kezdeti sebesség', 'Sebesség: ', 20, 5, 50)
    α = turtle.numinput('Hajítási szög', 'Hajítási szög: (0-nál ez keleti irányba mutat)', 10, 0)
    α = radian(α)
    Vx = V*cos(α)
    Vy = V*sin(α)
    g = 9.81
    ...
    x0 = 37.175
    y0 = 36.775
    x,y = pont_eltol(x0,y0,pi+α,0.5*V)
    ...
    while repul:
        ...
        x += t*Vx
        if not gyozelem:
            gyozelem = pont_szakasz_tav(malac_x,malac_y,x,y,elozo_x,elozo_y) <= 15
        if y <= 8:
            repul = False
            y = 8
        alakzat_kirajzol(taj,0,0)
